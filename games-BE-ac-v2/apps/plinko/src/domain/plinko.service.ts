...
import { PLINKO_MATH } from '../math/plinko';
...
@Injectable()
export class PlinkoService extends CommonService {
  constructor(
  ...
  async betPlace(input: PlinkoBetPlaceInput, currentUser: AuthUser): Promise<PlinkoBetPlaceResponseDTO> {
    const { gameMode, operatorId, token, playerId } = currentUser;
    const { currency, betAmount, risk, rows } = input;

    this.monitor.info(`===Plinko betPlace started ===`, { data: { input } });
    ...
    const payout = payInfo.multiplier * betAmount;
    ...
    this.rgsServiceInstance
      ...
      .then((resp) => {
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `Error updating userBet to status CREDIT_SUCCESS for plinko, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      })
      .catch((error) => {
        ...
        this.monitor.error('Error on RGS credit call from plinko betPlace', {
          error,
          data: { betId, roundId },
        });
        ...
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `Error updating userBet to status CREDIT_FAILED for plinko, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      });

    this.monitor.info(`===Plinko betPlace ended with data ===`, { data: { input } });
  ...
  async payTable(currentUser: AuthUser) {
    ...
    return PLINKO_MATH[gameMode].gameLevel;
  }
}
