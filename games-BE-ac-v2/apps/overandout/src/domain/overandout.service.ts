...
import { OVERANDOUT_MATH } from '../math/overandout';
...
import { IOverAndOutRound, OverAndOutUserBet } from './interfaces/IOverAndOutGameState';

@Injectable()
export class OverAndOutService extends CommonService {
  constructor(
    ...
    @InjectQueue(OVERANDOUT_QUEUE) private queue: Queue,
  ) {
    super();
  }
  ...
  private async getExistingBetOrFail(currentUser: AuthUser) {
    ...
    if (!existingBet) {
      this.monitor.error('OverAndOut bet not found.', { data: { currentUser } });
  ...
  async nextMine(position: number, currentUser: AuthUser): Promise<OverAndOutBetPlaceResponseDTO> {
    const { playerId, gameCode } = currentUser;

    if (gameCode !== GameCodes.OVERANDOUT) {
      this.monitor.error(
        'OverAndOut: nextMine: Invalid game code passed, please use the correct token assigned for this game.',
        { data: { gameCode, playerId } },
      );
      throw new ServerError({ context: this, code: GameBeResultCodes.INVALID_GAMECODE_MINES });
    }

    this.monitor.info('===OverAndOut: nextMine started for user ===', { data: playerId });

    let locker: Lock;
    try {
      const lockKey = `be:lock:overandout:cashout:${playerId}`;
      locker = await this.redlockClient.lock(lockKey, this.redlockConfigService.redlock.ttl);

      const bet: OverAndOutUserBet = await this.getExistingBetOrFail(currentUser);
      ...
      if (!isMineSelected) {
        const multiplierDetails = OVERANDOUT_MATH[gameMode].multiplierMap[state.mineCount - 1][state.rounds.length];
        payoutMultiplier = multiplierDetails.multiplier;

        if (state.rounds.length + state.mineCount + 1 === 24) {
          isGameEnded = true;
        }
      }

      const payout = betAmount * payoutMultiplier;
      const newRound: IOverAndOutRound = {
        field: position,
        payoutMultiplier,
      };
      ...
      const cacheKey = getRedisCacheKeys(RedisCacheKeyEnum.ACTIVE_BET, { playerId, gameCode: GameCodes.OVERANDOUT });
      ...
      this.userBetService
        ...
        .catch((e) =>
          this.monitor.error('OverAndOut: nextMine: Update bet for newRound', {
            error: e,
      ...
      if (isGameEnded || isMineSelected) {
        this.monitor.info('===OverAndOut:nextMine: Game comes to an end===', { data: { betId: bet.betId, playerId } });

        return this.cashOut(currentUser, bet);
      }

      this.monitor.info('===OverAndOut: nextMine ended for user ===', { data: { betId: bet.betId, playerId } });

      return {
        gameCode: GameCodes.OVERANDOUT,
        roundId: bet.roundId,
        betId: bet.betId,
  ...
  public async cashOut(currentUser: AuthUser, bet?: OverAndOutUserBet): Promise<OverAndOutBetPlaceResponseDTO> {
    const { playerId } = currentUser;

    this.monitor.info('===OverAndOut: cashOut started for user ===', { data: playerId });
    ...
    try {
      ...
      if (betStatus !== BetStatus.DEBIT_SUCCESS && betStatus !== BetStatus.CREDIT_FAILED) {
        this.monitor.warn('OverAndOut: cannot cashout as bet is not on DEBIT_SUCCESS/CREDIT_FAILED status', {
      ...
      const lastRound: IOverAndOutRound = bet.state.rounds.slice(-1).pop();
      ...
      this.rgsServiceInstance
        ...
        .then((resp) => {
          ...
          if (rgsData) {
            this.userBetService
              ...
              .catch((e) =>
                this.monitor.error(
                  `Error updating userBet to status CREDIT_SUCCESS for overandout, roundId: ${roundId}, betId: ${betId}`,
                  { error: e },
                ),
              );
          } else {
        ...
        .catch((error) => {
          ...
          this.monitor.error('Error on RGS credit call from overandout betPlace', {
            error,
            data: { betId, roundId },
          });
          ...
          this.userBetService
            ...
            .catch((e) =>
              this.monitor.error(
                `Error updating userBet to status CREDIT_FAILED for overandout, roundId: ${roundId}, betId: ${betId}`,
                { error: e },
              ),
            );
        });
      ...
      this.monitor.info('===OverAndOut: cashOut ended for user ===', {
  ...
  async betPlace(input: OverAndOutBetPlaceInput, currentUser: AuthUser): Promise<OverAndOutBetPlaceResponseDTO> {
    const { gameMode, operatorId, token, playerId } = currentUser;
    const { currency, betAmount, mineCount } = input;

    this.monitor.info(`===OverAndOut betPlace started ===`, { data: { input } });
    ...
    const userBetInfo: OverAndOutUserBet = {
      userId: user._id,
      playerId,
      operatorId,
      token,
    ...
    const initialTiles = Array.from(Array(24).keys());
    ...
    this.queue.add(QueueJobs.OVERANDOUT_SETTLE_BET, { betId }, { delay: 24 * 3600 * 1000, jobId: betId }); // This will check the job status after 24hrs, if the bet is still open then close the bet and refund the money.

    this.monitor.info(`===OverAndOut betPlace ended with data ===`, { data: { input, betId } });
  ...
  async payTable(currentUser: AuthUser) {
    ...
    return OVERANDOUT_MATH[gameMode].multiplierMap;
  }

  public async autoSettleBet(data: { betId: string }): Promise<void> {
    const { betId } = data;

    this.monitor.info('===OverAndOut: autoSettle bet started ===', { data: { betId } });

    try {
      const userBet = await this.userBetService.findBetCacheOrFallBack(betId);

      if (!userBet) {
        this.monitor.info('===OverAndOut: autoSettle bet, no user bet found ===', { data: { betId } });
        return;
      } else if (userBet.betStatus !== BetStatus.DEBIT_SUCCESS) {
        this.monitor.info('===OverAndOut: autoSettle bet, not in DEBIT_SUCCESS status ===', {
      ...
      this.monitor.info('===OverAndOut: autoSettle bet ended ===', { data: { betId } });
    } catch (error) {
      this.monitor.error('===OverAndOut: autoSettle bet error ===', { error, data: { betId } });
    }
  }

  public async autoBet(
    input: OverAndOutAutoBetPlaceInput,
    currentUser: AuthUser,
  ): Promise<OverAndOutBetPlaceResponseDTO> {
    const { gameMode, operatorId, token, playerId } = currentUser;
    const { currency, betAmount, mineCount, positions } = input;

    this.monitor.info(`===OverAndOut autoBet started ===`, { data: { input } });
    ...
    const multiplierDetails = OVERANDOUT_MATH[gameMode].multiplierMap[mineCount - 1];
    let payout = 0;
    let payoutMultiplier = 0;
    let rounds: IOverAndOutRound[] = [];
    ...
    this.rgsServiceInstance
      ...
      .then((resp) => {
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `autoBet: Error updating userBet to status CREDIT_SUCCESS for overandout, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      })
      .catch((error) => {
        ...
        this.monitor.error('autoBet: Error on RGS credit call from overandout betPlace', {
          error,
          data: { betId, roundId },
        });
        ...
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `autoBet: Error updating userBet to status CREDIT_FAILED for overandout, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      });

    this.monitor.info(`===OverAndOut autoBet ended with data ===`, { data: { input, betId } });
