...
import { ButtonPopHashCode, ButtonPopHashCodeModel } from './schema/buttonpopHashCode.schema';
import { ButtonPopRound, ButtonPopRoundModel } from './schema/buttonpopRounds.schema';
...
@Injectable()
export class ButtonPopService extends CommonService implements OnApplicationBootstrap {
  ...
  constructor(
    ...
    @InjectModel(ButtonPopRound.name) private roundModel: ButtonPopRoundModel,
    @InjectModel(ButtonPopHashCode.name) private hashCodeModel: ButtonPopHashCodeModel,
  ...
  onApplicationBootstrap() {
    this.monitor.info('Buttonpop service module init called');
    this.monitorGameModes();
  }

  private async getGameSeed() {
    let seeds = await this.localCacheService.getCache<GameSeeds>(`${GameCodes.BUTTONPOP}/gameSeeds`);

    if (!seeds) {
      this.monitor.info('===Buttonpop: fetching Game seed ===');

      seeds = await this.gameSeedsModel.findOne({ gameCode: GameCodes.BUTTONPOP }).select({ seed: 1 });

      if (!seeds) {
        seeds = await this.gameSeedsModel.create(GAME_SEEDS[GameCodes.BUTTONPOP]);
      }

      await this.localCacheService.setCache({
        key: `${GameCodes.BUTTONPOP}/gameSeeds`,
  ...
  private async runGameLoop(gameMode: GameMode) {
    ...
    const loop = async () => {
      if (!control || control.stop) {
        ...
        const globalRoom = getRoomNameByGame(GameCodes.BUTTONPOP, gameMode);
  ...
  private async updateGameCache(gameMode: GameMode, data: IActiveGameCache) {
    await this.cacheService.setCache({
      key: getRedisCacheKeys(RedisCacheKeyEnum.ACTIVE_GAME, { gameCode: GameCodes.BUTTONPOP, gameMode }),
      data,
      expire: 900,
    });
  }
  ...
  private async createGame(gameMode: GameMode): Promise<void> {
    this.monitor.info('===Buttonpop: createGame started ===', { data: { gameMode } });
    const globalRoom = getRoomNameByGame(GameCodes.BUTTONPOP, gameMode);

    const gameSeeds = await this.preGenerateServerCodeService.getSeeds<ButtonPopHashCode>(
      this.hashCodeModel,
      GameCodes.BUTTONPOP,
      2,
    );
    ...
    this.hashCodeModel
      ...
      .catch((e) =>
        this.monitor.error(`Error on Buttonpop server codes update for ID: ${gameSeedInfo._id}`, { error: e }),
      );

    this.monitor.info('===Buttonpop: adding new game===');

    const roundId = await this.generateUUid();
    const roundKey = getRedisCacheKeys(RedisCacheKeyEnum.BET_LIST, { gameCode: GameCodes.BUTTONPOP, roundId });

    const gameRound = await this.roundModel.create({
    ...
    this.monitor.info('===Buttonpop: new game added ===', {
    ...
    gameRound.updateOne({ status: MultiPlayerGameStates.ACCEPT_BET }).catch((e) =>
      this.monitor.error(`Error on Buttonpop gameRound update to accept bet for ID: ${gameRound.id}`, {
        error: e,
        data: { roundId },
      }),
    );

    this.monitor.info(`===Buttonpop: game ${gameRound._id}, roundId: ${roundId} updated to bet accepting state ===`);
    ...
    const totalBets = await this.cacheService.getCache<number>(
      getRedisCacheKeys(RedisCacheKeyEnum.TOTAL_BET_COUNT, { gameCode: GameCodes.BUTTONPOP, roundId }),
    );
    ...
    const autoCashOutLocalKey = `${GameCodes.BUTTONPOP}/${roundId}/autoCashOutLocal`;
    ...
    gameRound
      ...
      .catch((e) =>
        this.monitor.error(
          `Error updating DB for Buttonpop game ${gameRound.id}, roundId: ${roundId} to running status`,
          {
            error: e,
          },
        ),
    ...
    this.cacheService.batchDeleteKeys([
      getRedisCacheKeys(RedisCacheKeyEnum.ACTIVE_GAME, { gameCode: GameCodes.BUTTONPOP, gameMode }),
      getRedisCacheKeys(RedisCacheKeyEnum.TOTAL_BET_COUNT, { gameCode: GameCodes.BUTTONPOP, roundId }),
      roundKey,
    ]);
    ...
    this.monitor.info('===Buttonpop: createGame ended ===', { data: { gameMode } });
  }
  ...
  private async runGame(gameMode: GameMode, cacheData: IActiveGameCache, globalRoom: string) {
    ...
    this.monitor.info(
      `===Buttonpop: game: ${gameId}, roundId: ${roundId}, will be executed till crash at ${finalCrashMultiplier}===`,
    );
    ...
    while (multiplier <= finalCrashMultiplier) {
      ...
      if (multiplier > finalCrashMultiplier) {
        ...
        this.monitor.info(`===Buttonpop: game: ${gameId}, roundId: ${roundId} has been crashed===`);
  ...
  private async handleGameCrash(cacheData: IActiveGameCache) {
    const { gameId, roundId, finalCrashMultiplier } = cacheData;
    try {
      this.monitor.info(`===Buttonpop: _handleGameCrash started for game ${gameId}, roundId ${roundId}===`);
      ...
      this.roundModel.updateOne({ _id: cacheData.gameId }, { status: MultiPlayerGameStates.ENDED }).catch((error) =>
        this.monitor.error('Buttonpop: _handleGameCrash: Error on game round update DB.', {
          error,
          data: { gameId, roundId },
        }),
      );
      ...
      this.monitor.info(`===Buttonpop: _handleGameCrash ended for game ${gameId}, roundId ${roundId}===`);
    } catch (error) {
      this.monitor.error('Error on Buttonpop handleGameCrash', { error, data: { gameId, roundId } });
    }
  }
  ...
  public async getRoundInfo(roundId: string): Promise<any> {
    const roundInfo = await this.roundModel.findOne({ roundId, status: MultiPlayerGameStates.ENDED });
  ...
  private async cacheLastRound(round: ButtonPopRound) {
    try {
      const { gameMode, crashMultiplier, roundId } = round;
      const key = getRedisCacheKeys(RedisCacheKeyEnum.LAST_ROUND_LIST, { gameCode: GameCodes.BUTTONPOP, gameMode });
      const lastRoundDetails: ButtonPopLastRoundDTO = { crashMultiplier, roundId };
  ...
  public async getLastMultipliers(gameMode: GameMode): Promise<ButtonPopLastRoundDTO[]> {
    const key = getRedisCacheKeys(RedisCacheKeyEnum.LAST_ROUND_LIST, { gameCode: GameCodes.BUTTONPOP, gameMode });

    const data = await this.cacheService.getList({ key, start: 0, stop: this.maxLastRounds - 1 });

    let lastRounds: ButtonPopLastRoundDTO[] = [];
