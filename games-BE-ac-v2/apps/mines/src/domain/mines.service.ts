...
import { MINES_MATH } from '../math/mines';
...
import { IMinesRound, MinesUserBet } from './interfaces/IMinesGameState';

@Injectable()
export class MinesService extends CommonService {
  constructor(
    ...
    @InjectQueue(MINES_QUEUE) private minesQueue: Queue,
  ) {
    super();
  }
  ...
  private async getExistingBetOrFail(currentUser: AuthUser) {
    ...
    if (!existingBet) {
      this.monitor.error('Mines bet not found.', { data: { currentUser } });
  ...
  async nextMine(position: number, currentUser: AuthUser): Promise<MinesBetPlaceResponseDTO> {
    const { playerId, gameCode } = currentUser;

    if (gameCode !== GameCodes.MINES) {
      this.monitor.error(
        'Mines: nextMine: Invalid game code passed, please use the correct token assigned for this game.',
        { data: { gameCode, playerId } },
      );
      throw new ServerError({ context: this, code: GameBeResultCodes.INVALID_GAMECODE_MINES });
    }

    this.monitor.info('===Mines: nextMine started for user ===', { data: playerId });

    let locker: Lock;
    try {
      const lockKey = `be:lock:mines:cashout:${playerId}`;
      locker = await this.redlockClient.lock(lockKey, this.redlockConfigService.redlock.ttl);

      const bet: MinesUserBet = await this.getExistingBetOrFail(currentUser);
      ...
      const cacheKey = getRedisCacheKeys(RedisCacheKeyEnum.ACTIVE_BET, { playerId, gameCode: GameCodes.MINES });
      ...
      this.userBetService
        ...
        .catch((e) =>
          this.monitor.error('Mines: nextMine: Update bet for newRound', {
            error: e,
      ...
      if (isGameEnded || isMineSelected) {
        this.monitor.info('===Mines:nextMine: Game comes to an end===', { data: { betId: bet.betId, playerId } });

        return this.cashOut(currentUser, bet);
      }

      this.monitor.info('===Mines: nextMine ended for user ===', { data: { betId: bet.betId, playerId } });

      return {
        gameCode: GameCodes.MINES,
        roundId: bet.roundId,
        betId: bet.betId,
  ...
  public async cashOut(currentUser: AuthUser, bet?: MinesUserBet): Promise<MinesBetPlaceResponseDTO> {
    const { playerId } = currentUser;

    this.monitor.info('===Mines: cashOut started for user ===', { data: playerId });
    ...
    try {
      ...
      if (betStatus !== BetStatus.DEBIT_SUCCESS && betStatus !== BetStatus.CREDIT_FAILED) {
        this.monitor.warn('Mines: cannot cashout as bet is not on DEBIT_SUCCESS/CREDIT_FAILED status', {
      ...
      const lastRound: IMinesRound = bet.state.rounds.slice(-1).pop();
      ...
      this.rgsServiceInstance
        ...
        .catch((error) => {
          ...
          this.monitor.error('Error on RGS credit call from mines betPlace', {
            error,
            data: { betId, roundId },
          });
          ...
          this.userBetService
            ...
            .catch((e) =>
              this.monitor.error(
                `Error updating userBet to status CREDIT_FAILED for mines, roundId: ${roundId}, betId: ${betId}`,
                { error: e },
              ),
            );
        });
      ...
      const [deletedFromCache, removedFromQueue] = await Promise.all([
        ...
        this.minesQueue.remove(bet.betId),
      ]);

      this.monitor.info('===Mines: cashOut ended for user ===', {
  ...
  async betPlace(input: MinesBetPlaceInput, currentUser: AuthUser): Promise<MinesBetPlaceResponseDTO> {
    const { gameMode, operatorId, token, playerId } = currentUser;
    const { currency, betAmount, mineCount } = input;

    this.monitor.info(`===Mines betPlace started ===`, { data: { input } });
    ...
    const userBetInfo: MinesUserBet = {
      userId: user._id,
      playerId,
      operatorId,
      token,
    ...
    this.minesQueue.add(QueueJobs.MINES_SETTLE_BET, { betId }, { delay: 24 * 3600 * 1000, jobId: betId }); // This will check the job status after 24hrs, if the bet is still open then close the bet and refund the money.

    this.monitor.info(`===Mines betPlace ended with data ===`, { data: { input, betId } });
  ...
  public async autoSettleBet(data: { betId: string }): Promise<void> {
    ...
    this.monitor.info('===Mines: autoSettle bet started ===', { data: { betId } });

    try {
      const userBet = await this.userBetService.findBetCacheOrFallBack(betId);

      if (!userBet) {
        this.monitor.info('===Mines: autoSettle bet, no user bet found ===', { data: { betId } });
        return;
      } else if (userBet.betStatus !== BetStatus.DEBIT_SUCCESS) {
        this.monitor.info('===Mines: autoSettle bet, not in DEBIT_SUCCESS status ===', {
      ...
      this.monitor.info('===Mines: autoSettle bet ended ===', { data: { betId } });
    } catch (error) {
      this.monitor.error('===Mines: autoSettle bet error ===', { error, data: { betId } });
    }
  }

  public async autoBet(input: MinesAutoBetPlaceInput, currentUser: AuthUser): Promise<MinesBetPlaceResponseDTO> {
    const { gameMode, operatorId, token, playerId } = currentUser;
    const { currency, betAmount, mineCount, positions } = input;

    this.monitor.info(`===Mines autoBet started ===`, { data: { input } });
    ...
    this.rgsServiceInstance
      ...
      .then((resp) => {
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `autoBet: Error updating userBet to status CREDIT_SUCCESS for MINES, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      })
      .catch((error) => {
        ...
        this.monitor.error('autoBet: Error on RGS credit call from MINES betPlace', {
          error,
          data: { betId, roundId },
        });
        ...
        userBetRec
          ...
          .catch((e) =>
            this.monitor.error(
              `autoBet: Error updating userBet to status CREDIT_FAILED for MINES, roundId: ${roundId}, betId: ${betId}`,
              { error: e },
            ),
          );
      });

    this.monitor.info(`===Mines autoBet ended with data ===`, { data: { input, betId } });
