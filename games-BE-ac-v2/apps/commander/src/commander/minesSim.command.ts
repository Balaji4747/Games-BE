import { MINES_MATH, MinesMath } from '@provfair/apps/mines/src/math/mines';
...
@Command({ name: 'minesSim' })
export class MinesSimCommand extends CommandRunner {
  ...
  private async sim(nonce: number) {
    ...
    if (minesPositions.some((mp) => selectBox.includes(mp))) {
      ...
      const mathData = MINES_MATH[this.gameMode].multiplierMap[this.minesSteps - 1];
  ...
  private createMath() {
    ...
    const workbook = XLSX.readFile('./maths/Limbo_Mine.xlsx');
    ...
    for (const gameMode in gameModesSheetMap) {
      if (Object.prototype.hasOwnProperty.call(gameModesSheetMap, gameMode)) {
        ...
        let c1R1 = 4;
        let l1R1 = 7;
        const startRange = (col1: number, col2: number) => `D${col1}:AA${col2}`;
        ...
        for (let i = 0; i < 25; i++) {
          ...
          for (let index = 0; index < rawData[0].length; index++) {
            ...
            formattedData.push({
              gem: rawData[0][index],
              probability: rawData[1][index],
              multiplier: rawData[3][index],
            });
          }
    ...
    writeFile('minesMath.json', JSON.stringify(finalData, null, 2), 'utf8', (err) => {
      if (err) throw err;
